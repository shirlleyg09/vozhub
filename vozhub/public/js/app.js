/**
 * VozHub — App v3
 * Bot colaborativo: rádio + MP3 upload + link direto + SoundCloud + YouTube
 * Fila: qualquer usuário do canal adiciona/controla (igual bot Discord)
 */
...
socket.on('connect_error', () => {
  const st = document.getElementById('conn-status');
  st.textContent = '❌ Servidor offline — rode: npm start'; st.className = 'conn-status err';
  document.getElementById('login-btn').disabled = false;
});
socket.on('disconnect', () => toast('⚠️ Desconectado. Reconectando...'));
...
async function doLogin() {
  ...
  av.className = 'up-av ' + avc(v);
  av.innerHTML = ini(v) + '<div class="up-dot"></div>';
...
socket.on('channel:users', ({ key, users, music }) => {
  ...
  badge.textContent = cnt; badge.style.display = cnt ? 'inline' : 'none';
});
...
socket.on('music:searching', ({ source }) => {
  const el = source === 'soundcloud' ? 'sc-results' : 'yt-results';
  document.getElementById(el).innerHTML = '<div class="search-loading">🔍 Buscando...</div>';
});
...
function rChannelPanel() {
  ...
  srv.channels.forEach((ch, i) => {
    ...
    row.innerHTML = '<span class="ch-icon">🔊</span><span class="ch-name">' + ch.name + '</span>' +
      (count ? '<span class="ch-badge">' + count + '</span>' : '');
  ...
  ban.innerHTML = `<div class="stg-vis">${srv.icon}</div><div class="stg-info">
    ...
    <div class="stg-acts">
      ${!S.connected
        ...
        : '<button class="stg-btn s" onclick="leaveVoice()">🚪 Sair</button>'}
...
function updateMusicUI() {
  ...
  // Thumbnail
  if (t.thumbnail) {
    th.innerHTML = `<img src="${t.thumbnail}" alt="${t.title}" onerror="this.parentElement.textContent='${t.emoji||'🎵'}'">`;
  } else {
    th.textContent = t.emoji || '🎵';
  }
  ...
  if (m.playing && !m.paused && t.duration > 0) {
    progIv = setInterval(() => { localProg = Math.min(localProg + 0.25, t.duration); updProg(); }, 250);
  }

  // Tocar áudio localmente
  let streamUrl = t.streamUrl || t.url || '';
  if (t.type === 'youtube' && t.url) streamUrl = `/api/ytstream?url=${encodeURIComponent(t.url)}`;
  if (t.type === 'soundcloud' && t.url) streamUrl = `/api/scstream?url=${encodeURIComponent(t.url)}`;

  if (m.playing && !m.paused && streamUrl) {
    if (audio.dataset.url !== streamUrl) {
      audio.src = streamUrl; audio.dataset.url = streamUrl;
...
function toggleMuteLocal() {
  ...
  toast(S.localMuted ? '🔇 Música silenciada localmente' : '🔈 Música ativada');
}
...
function updateSourceBadges() {
  ...
  if (S.sources.soundcloud) {
    scSt.className = 'source-status ok'; scSt.textContent = '✅ SoundCloud disponível';
  } else {
    scSt.className = 'source-status warn';
    scSt.textContent = '⚠️ SoundCloud inativo — rode: npm install soundcloud-scraper';
  }
  // YouTube
  const ytSt = document.getElementById('yt-status');
  if (S.sources.youtube) {
    ytSt.className = 'source-status ok'; ytSt.textContent = '✅ YouTube disponível';
  } else {
    ytSt.className = 'source-status warn';
    ytSt.textContent = '⚠️ YouTube inativo — rode: npm install ytdl-core yt-search';
  }
}
...
function renderRadios(filter = '') {
  ...
  const currentUrl = S.music?.track?.url || '';
  const list = filter ? RADIOS.filter(r => r.name.toLowerCase().includes(filter.toLowerCase()) || r.genre.toLowerCase().includes(filter.toLowerCase())) : RADIOS;
  list.forEach(r => {
    const card = document.createElement('div');
    card.className = 'radio-card' + (currentUrl === r.url ? ' playing' : '');
  ...
  if (!list.length) grid.innerHTML = '<div style="color:var(--t3);padding:20px;text-align:center">Nenhuma rádio encontrada</div>';
}
...
function handleDrop(e) {
  ...
  else toast('❌ Apenas arquivos de áudio (MP3, OGG, AAC...)');
}
...
function addDirectUrl() {
  ...
  if (!url) { toast('❌ Digite uma URL de áudio'); return; }
...
/* Renderizar resultados de busca (SC e YT) */
function renderSearchResults(containerId, results, error, source) {
  ...
  if (!results?.length) { c.innerHTML = '<div class="search-loading">Nenhum resultado encontrado.</div>'; return; }
  results.forEach(r => {
    const item = document.createElement('div'); item.className = 'search-item';
    item.innerHTML = `
      <div class="si-thumb">${r.thumbnail ? `<img src="${r.thumbnail}" alt="">` : (r.emoji || (source==='sc'?'☁️':'▶️'))}</div>
      <div class="si-info">
        <div class="si-title">${r.title}</div>
        <div class="si-meta">${r.artist || ''} · ${r.durationFmt || '?'} · ${source === 'sc' ? '☁️ SoundCloud' : '▶️ YouTube'}</div>
      </div>
...
function renderQueue() {
  ...
  sub.textContent = cnt + ' faixa' + (cnt !== 1 ? 's' : '') + ' na fila' + (m?.shuffled ? ' · 🔀 Aleatório' : '');
  ...
  if (cur) {
    ...
    now.innerHTML = `<span class="qnp-emoji">${cur.emoji || '🎵'}</span>
      <div class="qnp-info">
        <div class="qnp-title">${cur.title}</div>
        <div class="qnp-meta">${cur.artist || ''} · por ${cur.requestedBy || '?'}</div>
      </div>
      <span class="qnp-badge">${m.paused ? '⏸ Pausado' : '▶ Tocando'}</span>`;
  }

  if (!queue.length) {
    list.innerHTML = `<div class="q-empty"><div class="q-empty-icon">🎵</div>
      Fila vazia — qualquer usuário do canal pode adicionar músicas!</div>`;
    return;
  }
  ...
  queue.forEach((t, i) => {
    ...
    item.innerHTML = `
      <span class="q-pos">${isCur ? '▶' : i + 1}</span>
      <span class="q-emoji">${t.emoji || '🎵'}</span>
      <div class="q-info">
        <div class="q-title${isCur ? ' cur' : ''}">${t.title}</div>
        <div class="q-meta">${t.isLive ? '🔴 Ao vivo' : fmt(t.duration || 0)} · <span class="q-by">por ${t.requestedBy || '?'}</span></div>
      </div>
      <div class="q-actions">
        <button class="q-act" title="Tocar agora">▶</button>
        <button class="q-act del" title="Remover da fila">✕</button>
      </div>`;
...
async function openSettings() {
  ...
  if (!inputs.length)  ms.innerHTML = '<option>Nenhum microfone encontrado</option>';
